...
def set_h(lstm, index, value):
    ...
    """
    lstm_value = K.get_value(lstm.value)
    lstm_flag  = K.get_value(lstm.flag)
    lstm_value[index] = value
    lstm_flag  = np.zeros_like(lstm_value)
    lstm_flag[index] = 1
    # print(bias[num_units*3 + offset])
    # batch_set_value takes in a list of tuples
    K.set_value(lstm.value, lstm_value)
    K.set_value(lstm.flag, lstm_flag)
    #check_bias = B.batch_get_value(lstm.bias)

    #print('check bias')
    #print(check_bias[num_units*3:])
...
class MYLSTM(Recurrent):
    ...
    def step(self, inputs, states):
        ...
        h = (K.ones_like(self.flag) - self.flag) * o * self.activation(c) + self.flag * self.value
